...
bmap, cmap, wmap, revmap, span_data, span_size, target_pred_ix_set, We = pd.read_pickle(parent_dir + "/data/large_model_input_gs1.pkl")
...
descriptor_log = parent_dir + '/LARN-origin/descriptors_model_' + model_name + '.log'
model_object_file_name = parent_dir + f"/LARN-origin/trained_model_larn.pt"
model = torch.load(model_object_file_name)

target_word_ix_set = set()
# target_word_ix_counter shows the counts of predicates 
target_word_ix_counter = pd.read_pickle(parent_dir + "/data/target_word_ix_counter.pk")
...
with torch.no_grad():
    ...
    desc_sample_file_name = parent_dir + '/LARN-origin/desc_selected_sample_dict_model_' + model_name + '.pkl'
    trajectory_log = parent_dir + '/LARN-origin/trajectories_model_' + model_name + '.log'
    model_object_file_name = parent_dir + f"/LARN-origin/trained_model_larn.pt"
    ...
    for book in rmn_traj:
        ...
        plt.show()
        #plt.savefig(plot_save_path + f'LARN-ori-TempTrend-{cmap[chars[0]]}-{cmap[chars[1]]}-{tpnr}.png', bbox_inches='tight')

        desc_share_df.to_pickle(model_save_path + f"LARN-ori-TimeSeries-{cmap[chars[0]]}-{cmap[chars[1]]}.pk")
